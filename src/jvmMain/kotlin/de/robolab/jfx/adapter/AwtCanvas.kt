...
class AwtCanvas(
    ...
    private val bufferedImage = BufferedImage((width * scale).toInt(), (height * scale).toInt(), BufferedImage.TYPE_INT_ARGB)
    ...
    override fun dashLine(points: List<Point>, color: Color, width: Double, dashes: List<Double>, dashOffset: Double) {
        context.paint = color.toPaint()
        context.stroke = BasicStroke(width.toFloat(), BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER, 10.0f, dashes.map { it.toFloat() }.toFloatArray(), dashOffset.toFloat())
    ...
    init {
        context.addRenderingHints(mutableMapOf(
