...
abstract class ComponentWithRegex(components: List<IRegexComponent> = emptyList()) : IRegexComponent {
    ...
    fun literal(literal:String){components.add(LiteralComponent(literal))}
    fun compiled(regex: Regex) {components.add(CompiledComponent(regex))}
    ...
    fun empty() {components.add(EmptyComponent)}
    fun epsilon() {components.add(EpsilonComponent)}
    fun unmatchable() {components.add(UnmatchableComponent)}

    fun optional(block: MultipleComponent.() -> Unit)
            = maybe(block)
    fun maybe(block: MultipleComponent.() -> Unit)
            = addAndRun(MultipleComponent(0,1, explicit = false), block)
    ...
    fun characterClass(block: CharacterClassComponent.() -> Unit)
            = addAndRun(CharacterClassComponent(), block)
    ...
    fun anyCharacter(){components.add(AnyCharacterComponent)}
