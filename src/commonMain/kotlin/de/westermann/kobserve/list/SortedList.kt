...
class SortedList<T>(
    ...
    override fun updateRelation() {
        if (parent.size != relation.size) {
            relation.clear()
            relation.addAll((0 until parent.size).map { Relation(it, parent[it].hashCode(), parent[it]) })
        }
    ...
    private fun mergeHalves(
        workA: MutableList<Relation<T>>,
        workB: MutableList<Relation<T>>,
        start: Int,
        mid: Int,
        exclusiveEnd: Int
    ) {
    ...
    private fun sortSection(
        input: MutableList<Relation<T>>,
        output: MutableList<Relation<T>>,
        start: Int,
        exclusiveEnd: Int
    ) {
