...
class TransformList<P, T>(
    ...
    override val onRemove: EventHandler<ListRemoveEvent<T>> = EventHandler()
    ...
    init {
        ...
        parent.onRemove { (index, element) ->
            var elem: T? = null
            if (useCache) {
                elem = cache.remove(index)
                cache.keys.filter {
                    it > index
                }.sorted().forEach {
                    val oldCache = cache[it]
            ...
            onRemove.emit(ListRemoveEvent(index, elem ?: transform(element)))
        }
    }
}
